...
extension TreeSitterClient
{
  ...
  func queryHighlightsForRangeAsync(
    range: NSRange,
    completion: @escaping (([HighlightRange]) -> Void)
  )
  {
  ...
  func queryLayerHighlights(
    layer: LanguageLayer,
    textView _: HighlighterTextView,
    range: NSRange
  ) -> [HighlightRange]
  {
    ...
    guard let cursor = layer.languageQuery?.execute(node: rootNode, in: tree)
    else
    {
      return []
    }
  ...
  func highlightsFromCursor(cursor: ResolvingQueryCursor, includedRange: NSRange) -> [HighlightRange]
  {
