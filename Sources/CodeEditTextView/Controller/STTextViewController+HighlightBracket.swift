...
extension STTextViewController
{
  ...
  private func highlightRange(_ range: NSTextRange, scrollToRange: Bool = false)
  {
    ...
    switch bracketPairHighlight
    {
      case .flash:
        ...
        layer.opacity = 0.0
      case let .bordered(borderColor):
        layer.borderColor = borderColor.cgColor
        layer.cornerRadius = 2.5
        layer.borderWidth = 0.5
        layer.opacity = 1.0
      case let .underline(underlineColor):
        layer.lineWidth = 1.0
        layer.lineCap = .round
  ...
  {
    guard let textRange = NSTextRange(range, provider: textView.textContentManager)
    else
    {
      return
    }
