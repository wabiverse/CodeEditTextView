...
class Highlighter: NSObject
{
  ...
  {
    0 ..< (textView.textContentStorage?.textStorage?.length ?? 0)
  }
  ...
  )
  {
    ...
    guard textView.textContentStorage?.textStorage != nil
    else
    {
...
private extension Highlighter
{
  ...
  func highlight(range rangeToHighlight: NSRange)
  {
    ...
    highlightProvider?.queryHighlightsFor(textView: textView,
    ...
    { [weak self] highlightRanges in
      ...
      guard self?.visibleSet.intersects(integersIn: rangeToHighlight) ?? false
      else
      {
        return
      }
      ...
      guard let textContentManager = textView.textLayoutManager.textContentManager,
            let textRange = NSTextRange(rangeToHighlight, provider: textContentManager)
      else
      {
        return
      }
...
//                                                                       provider: textView.textContentStorage)!)
  ...
  {
    let set = IndexSet(integersIn: entireTextRange) // All text
...
private extension Highlighter
{
  ...
  @objc func visibleTextChanged(_: Notification)
  {
    updateVisibleSet()
...
extension Highlighter: NSTextStorageDelegate
{
                   ...
                   changeInLength delta: Int)
  {
    ...
    highlightProvider?.applyEdit(textView: textView,
                                 range: range,
                                 delta: delta)
    { [weak self] invalidatedIndexSet in
